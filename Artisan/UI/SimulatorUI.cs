...
namespace Artisan.UI
{
    public static class SimulatorUI
    {
        ...
        {
            ImGuiEx.TextWrapped($"在这个模拟器中，你可以根据配方测试不同的求解器，并分析它们的性能。你可以设置你的HQ素材配比，设置消耗品，甚至使用哪个装备套装。模拟器可以配置为随机化条件或仅使用\"Normal\"/\"正常\"条件，因此实操效果可能会有所不同。");
        }
        ...
        private static void DrawExports()
        {
            if (SimActionIDs.Count > 0 && (_simCurSolver is not MacroSolver || inManualMode) && !hoverMode)
            {
                ...
                ImGui.SameLine();
                if (ImGui.Button($"导出为宏"))
                {
                    if (string.IsNullOrEmpty(macroName))
                    {
                        Notify.Error("请提供宏的名称");
                        return;
                    }
        ...
        private static void DrawSolverActions()
        {
            if (_selectedSolver != null && (SimGS != null || CustomStatMode))
            {
                ...
                ImGui.SameLine();
                if (ImGui.Checkbox($"仅假设为普通状态", ref assumeNormalStatus))
                {
                    _selectedCraft = Crafting.BuildCraftStateForRecipe(SimStats, Job.CRP.Add(SelectedRecipe.Value.CraftType.RowId), SelectedRecipe.Value);
                    _simCurSteps.Clear();
                }
        ...
        private static void DrawSimResult()
        {
            ...
            ImGuiEx.LineCentered($"SimResults", () => ImGuiEx.TextUnderlined($"模拟器结果 - {status.ToOutputString()}"));
        ...
        private static void DrawStatInfo()
        {
            if (SimGS != null)
            {
                ImGuiEx.LineCentered("SimulatorStats", () => ImGuiEx.TextUnderlined("制作师属性"));
                ...
                ImGui.NextColumn();
                ImGui.TextWrapped($"卓越/宇宙工具：{gsStats.SplendorCosmic}");
                ImGui.NextColumn();
                ImGui.TextWrapped($"专家：{gsStats.Specialist}");
                ImGui.NextColumn();
                ImGui.TextWrapped($"专业技能解锁：{gsStats.Manipulation}");
                ImGui.Columns(1);
        ...
        private static unsafe void DrawGearSetDropdown()
        {
            if (!CustomStatMode)
            {
                if (ImGui.Button($"切换到自定义属性模式", new (ImGui.GetContentRegionAvail().X, 0)))
                    CustomStatMode = true;
            }
            else
            {
                if (ImGui.Button($"切换到装备模式", new(ImGui.GetContentRegionAvail().X, 0)))
            ...
            if (!CustomStatMode)
            {
                ...
                if (validGS == 0)
                {
                    ImGuiEx.Text($"请为 {LuminaSheets.ClassJobSheet[SelectedRecipe.Value.CraftType.RowId + 8].Name} 添加一个装备套装");
                    SimGS = null;
                    return;
                }
                if (validGS == 1)
            ...
            else
            {
                ...
                ImGui.NextColumn();
                ImGui.Columns(3, border: false);
                ImGUIMethods.FlippedCheckbox($"卓越/宇宙：", ref gsSplend);
                ImGui.NextColumn();
                ImGUIMethods.FlippedCheckbox($"专家：", ref gsSpecialist);
                ImGui.NextColumn();
                ImGUIMethods.FlippedCheckbox($"已解锁掌握：", ref gsManip);
                ImGui.Columns(1);
        ...
        private static void DrawRecipeSelector()
        {
            var preview = SelectedRecipe is null
                                      ? string.Empty
                                      : $"{SelectedRecipe?.ItemResult.Value.Name.ToDalamudString().ToString()} ({LuminaSheets.ClassJobSheet[SelectedRecipe.Value.CraftType.RowId + 8].Name.ToString()})";
            ...
            if (ImGui.BeginCombo("###SimulatorRecipeSelect", preview))
            {
                try
                {
                    ...
                    foreach (var recipe in LuminaSheets.RecipeSheet.Values.Where(x => x.ItemResult.Value.Name.ToDalamudString().ToString().Contains(Search, StringComparison.CurrentCultureIgnoreCase)))
                    {
                        ...
                        var selected = ImGui.Selectable($"{recipe.ItemResult.Value.Name.ToDalamudString().ToString()} ({LuminaSheets.ClassJobSheet[recipe.CraftType.RowId + 8].Name.ToString()} {recipe.RecipeLevelTable.Value.ClassJobLevel})", recipe.RowId == SelectedRecipe?.RowId);
